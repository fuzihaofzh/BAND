...
def evaluate(model, batches, tot_gold, out_dir = None):
    """
    ...
    if out_dir: #maple
        json.dump({"P" : p, "R" : r, "F1" : f1}, open(f"{out_dir}/eval_score.json", "w"))
    return f1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--task', type=str, default=None, required=True, choices=['ace04', 'ace05', 'scierc', "ncbi", "band"])
    ...
    dev_samples, dev_ner = convert_dataset_to_samples(dev_data, args.max_span_length, ner_label2id=ner_label2id, context_window=args.context_window, part = "dev")
    dev_batches = batchify(dev_samples, args.eval_batch_size)

    if args.do_train:
        train_data = Dataset(args.train_data)
        train_samples, train_ner = convert_dataset_to_samples(train_data, args.max_span_length, ner_label2id=ner_label2id, context_window=args.context_window, part = "train")
        ...
        for _ in tqdm(range(args.num_epoch)):
            ...
            for i, batch in pbar:
                train_batches[i] = [item for item in train_batches[i] if item['sent_length'] < 280]# maple
                if len(train_batches[i]) == 0:
                    continue
    ...
    if args.do_eval:
        ...
        test_samples, test_ner = convert_dataset_to_samples(test_data, args.max_span_length, ner_label2id=ner_label2id, context_window=args.context_window, part = "test")
        test_batches = batchify(test_samples, args.eval_batch_size)
        evaluate(model, test_batches, test_ner, args.output_dir)
