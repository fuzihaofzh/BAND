...
class BertForEntity(BertPreTrainedModel):
    ...
    def _get_span_embeddings(self, input_ids, spans, token_type_ids=None, attention_mask=None):
        ...
        """if input_ids.shape[1] > 512:
            ...
            spans = spans.clamp_max(511)
            spans[:, :, 2] = spans[:, :, 1] - spans[:, :, 0] + 1"""
        tmp = self.bert(input_ids=input_ids, token_type_ids=token_type_ids, attention_mask=attention_mask)
        sequence_output, pooled_output = tmp[0], tmp[1] #maple
...
class EntityModel():
    ...
    def run_batch(self, samples_list, try_cuda=True, training=True):
        ...
        if training:
            ...
            if tokens_tensor.shape[1] > 512:#maple
                output_dict['pred_ner'] = [[]]
                output_dict['ner_probs'] = [[]]
                output_dict['ner_last_hidden'] = [[]]
                return output_dict
            with torch.no_grad():
