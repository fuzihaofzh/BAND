...
def main():
    ...
    if "ptm" in usermode.usermode:
        if usermode.usermode["ptm"] == "t5":
            model_args.model_name_or_path = "t5-base"
        elif usermode.usermode["ptm"] == "t5large":
            model_args.model_name_or_path = "t5-large"
            training_args.fp16 = False
            #training_args.num_train_epochs = 10
        elif usermode.usermode["ptm"] == "flant5":
            model_args.model_name_or_path = "google/flan-t5-base"
            training_args.fp16 = False
            #training_args.num_train_epochs = 10
        elif usermode.usermode["ptm"] == "ul2": # So large
            model_args.model_name_or_path = "google/ul2"
        elif usermode.usermode["ptm"] == "flanul2": # So large
            model_args.model_name_or_path = "google/flan-ul2"
    ...
    if data_args.dataset_name is not None:
        ...
        if "squad" in data_args.dataset_name:#maple
            raw_datasets["test"] = raw_datasets["validation"]#.select(range(100))
    else:
        data_files = {}
    ...
    if training_args.do_predict:
        ...
        #==maple
        anspair = [{"gold" : l['answers']['text'], "pred" : p['prediction_text']} for l, p in zip(results.label_ids, results.predictions)]
        json.dump(anspair, open(f"{training_args.output_dir}/pred.json", "w"), indent=2)
        #==
